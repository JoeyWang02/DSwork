...
void Floyd(int start, int end)
{
    ...
    } else
    {
        ...
        if (pre[start - 1][end - 1] == end - 1){ // 如果可以直连
            ...
            while (temp != end - 1)
            {
                printf(" --(%dm)-> %s",ZJUMap.path[temp][pre[temp][end - 1]], ZJUMap.place[pre[temp][end - 1]].name);
                temp = pre[temp][end - 1];
            }
        }
        printf("\n");
...
void quit()  //退出
{
    if (isAdmin == true)
        rewriteMap();
    printf("\033[1;30;43m\nMap Saved!!\n");
    printf("Successfully exited, welcome to use Zhejiang University Navigation System again!");
    printf("\033[0m\n"); // 恢复默认样式
    exit(0);
}
